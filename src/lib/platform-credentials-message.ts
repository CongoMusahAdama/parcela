...
export function platformCredentialSuccessText(result: PlatformCredentialResult): string {
  if (result.smsSent) {
    return `Temporary login details were sent by SMS to the phone on file for ${result.email}. They were reminded not to share the code with anyone.`;
  }
    return `SMS could not be sent. Share this temporary code with ${result.email}: ${result.temporaryPassword}. Tell them not to share it with anyone.`;
}
